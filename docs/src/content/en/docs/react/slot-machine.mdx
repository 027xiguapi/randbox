...
interface SlotMachineProps {
  ...
  // Optional properties
  weights?: number[][];                             // Symbol weights for each reel
  animationDuration?: number;                       // Animation duration (ms), default 3000
  buttonText?: string;                              // Button text, default "Start"
  ...
  onGameEnd?: (result: SlotMachineResult) => void; // Game end callback
  onResult?: (result: SlotMachineResult) => void;  // Result callback
}
```
...
interface SlotMachineResult {
  ...
  isJackpot: boolean;     // Whether it's a jackpot (all symbols same)
