...
interface DiceGameProps {
  ...
  gameMode?: DiceGameMode;                     // Game mode, default 'simple'
  targetSum?: number;                          // Target sum (for sum mode)
  ...
  onGameEnd?: (result: DiceGameResult) => void; // Game end callback
  onResult?: (result: DiceGameResult) => void; // Result callback
}
```
...
```typescript
type DiceGameMode =
  | 'simple'        // Simple mode, only shows dice results
  | 'sum'           // Sum mode, shows sum of all dice
  | 'bigSmall'      // Big/Small mode, guess big or small
  | 'guess'         // Guess mode, guess specific number
  | 'even_odd'      // Even/Odd mode, guess odd or even
  | 'specific';     // Specific mode, guess if specific number appears
```
...
interface DiceGameResult {
  results: number[];    // Array of all dice results
  total: number;        // Sum of dice values
  gameMode: string;     // Game mode
  isWin?: boolean;      // Whether won (in competitive mode)
  message: string;      // Result description
  values: number[];     // All dice values (same as results)
  sum: number;          // Sum (same as total)
