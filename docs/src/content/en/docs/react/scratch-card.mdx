...
interface ScratchCardProps {
  ...
  symbols?: string[];                                // Symbol array for filling cards
  winProbability?: number;                           // Win probability (0-1), default 0.3
  ...
  onGameEnd?: (result: ScratchCardResult) => void; // Game end callback
  onScratch?: (result: ScratchCardResult) => void; // Scratch callback
...
interface ScratchCardResult {
  ...
  winningInfo?: {                  // Win information
    pattern: string[];             // Win pattern (line type)
    name: string;                 // Win name
    prize: string;                 // Prize name
    symbol?: string;               // Win symbol
    type?: 'row' | 'col' | 'diagonal'; // Win type: row/column/diagonal
    positions: Array<{             // Win position array
      row: number;
      col: number;
    }>;
  };
