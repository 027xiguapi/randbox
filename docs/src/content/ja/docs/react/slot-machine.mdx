...
interface SlotMachineProps {
  // 必需属性
  reels: string[][];                                 // 滚轴数组，每个滚轴包含多个符号

  // 可选属性
  weights?: number[][];                              // 每个滚轴符号的权重
  animationDuration?: number;                        // 动画时长（毫秒），默认3000
  buttonText?: string;                               // 按钮文字，默认"开始"
  ...
  onGameEnd?: (result: SlotMachineResult) => void; // 游戏结束回调
  onResult?: (result: SlotMachineResult) => void;  // 结果回调
}
```
...
interface SlotMachineResult {
  ...
  isJackpot: boolean;        // 是否中大奖（所有符号相同）
