...
test('cc() passes the luhn algorithm', t => {
    _.times(1000, () => {
        let number = randBox.cc()
        t.true(randBox.luhn_check(number))
    })
})

test('cc() can take a type arg and obey it', t => {
    _.times(1000, () => {
        let type = randBox.cc_type({ raw: true })
        let number = randBox.cc({ type: type.name })
...
test('cc() can take a short_name type arg and obey it', t => {
    _.times(1000, () => {
        let type = randBox.cc_type({ raw: true })
        let number = randBox.cc({ type: type.short_name })
        t.is(number.length, type.length)
    })
})
...
test('cc_type() returns a random credit card type', t => {
    _.times(1000, () => {
        t.true(_.isString(randBox.cc_type()))
    })
})
...
test('cc_type() can take a name arg and obey it', t => {
    _.times(1000, () => {
        let type = randBox.cc_type({ name: 'Visa', raw: true })
        t.is(type.name, 'Visa')
    })
})

test('cc_type() with bogus type throws', t => {
    const fn = () => randBox.cc_type({ name: 'potato' })
    t.throws(fn, {message: 'RandBox: Credit card type \'potato\' is not supported'})
})

// randBox.cc_types()
test('cc_types() returns an array of credit card types', t => {
    t.true(_.isArray(randBox.cc_types()))
})

// randBox.currency()
test('currency() returns a currency', t => {
    _.times(1000, () => {
        let currency = randBox.currency()
...
test('currency_pair() returns a currency pair', t => {
    _.times(1000, () => {
        let currency_pair = randBox.currency_pair()
...
test('currency_pair() can return string version', t => {
    _.times(1000, () => {
        let currency_pair = randBox.currency_pair(true)
...
test('dollar() obeys min and max, if provided', t => {
    _.times(1000, () => {
        let dollar = randBox.dollar({ max: 20 })
        let dollarFloat = parseFloat(dollar.substring(1, dollar.length))
        t.true(dollarFloat <= 20)
    })

    _.times(1000, () => {
        let dollar = randBox.dollar({ min: 20 })
        let dollarFloat = parseFloat(dollar.substring(1, dollar.length))
        t.true(dollarFloat >= 20)
    })
})
...
test('dollar() can take negative min and max', t => {
    _.times(1000, () => {
        let dollar = randBox.dollar({ min: -200, max: -1 })
...
test('exp_month() will return a future month if requested', t => {
    _.times(1000, () => {
        ...
        let expMonth = parseInt(randBox.exp_month({ future: true }), 10)
        if (nowMonth !== 12) {
            t.true(expMonth > nowMonth)
        } else {
            t.true(expMonth >= 1)
...
test('luhn_check() properly checks if number passes the Luhn algorithm', t => {
    t.true(randBox.luhn_check(49927398716))
    t.true(randBox.luhn_check(1234567812345670))
    t.false(randBox.luhn_check(49927398717))
    t.false(randBox.luhn_check(1234567812345678))
})

test('iban() returns an iban', t => {
    let iban = randBox.iban()
