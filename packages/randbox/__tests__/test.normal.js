...
test('normal() returns values fairly close to the expected standard deviation and mean', t => {
    let testStddev = 1
    let testMean = 0
    let group = randBox.n(randBox.normal, 10000)
...
test('normal() works as expected with a pool of custom values provided', t => {
    ...
    _.times(1000, () => {
        let norm = randBox.normal({ mean: testMean, dev: testStddev, pool: pool })
...
test('normal() can be used with other randBox functions', t => {
    let testStddev = 1
    let testMean = 3
    let group = randBox.n(randBox.normal, 1000, { mean: testMean, dev: testStddev, pool: pool })
...
test('normal() should produce a correctly distributed group of pool items', t => {
    let testStddev = 2
    let testMean = 6
    let group = randBox.n(randBox.normal, 10000, { mean: testMean, dev: testStddev, pool: pool })
...
test('normal() should throw an error quickly if the user has provided bad pool', t => {
    let testStddev = 5
    let testMean = 200
    const fn = () => randBox.normal({ mean: testMean, dev: testStddev, pool: pool })
    t.throws(fn, {message: 'RandBox: Your pool is too small for the given mean and standard deviation. Please adjust.'})
})
...
test('normal() should throw an error if called with non-number mean', t => {
    ...
    let testMean = []
    const fn = () => randBox.normal({ mean: testMean, dev: testStddev, pool: pool })
    t.throws(fn, {message: 'RandBox: Mean (mean) must be a number'})
})
...
test('normal() should throw an error if called with non-number stddev', t => {
    ...
    let testMean = 5
    const fn = () => randBox.normal({ mean: testMean, dev: testStddev, pool: pool })
    t.throws(fn, {message: 'RandBox: Standard deviation (dev) must be a number'})
})

test('normal() should throw an error if the pool provided is not an array', t => {
    const fn = () => randBox.normal({ pool: 'not an array' })
    t.throws(fn, {message: 'RandBox: The pool option must be a valid array.'})
})

test('normal() should work with objects', t => {
    let testStddev = 1
    let testMean = 1
    let group = randBox.n(randBox.normal, 50, { mean: testMean, dev: testStddev, pool: [
        { a: 1, b: 10},
        { a: 2, b: 20},
        { a: 3, b: 30}
    ]})
